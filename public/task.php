...
$query = PDB::$conn->prepare('SELECT * FROM pro_tasks WHERE block = 0 ORDER BY `date` DESC, `idx` DESC LIMIT 0, 1');
$query->execute();
...
<body>
    ...
    <script>
        var code = CodeMirror.fromTextArea($('textarea')[0], {
            indentUnit: 4,
            mode: 'clike',
            lineNumbers: true,
            theme: 'mdn-like',
    ...
    /* Source Editor: Utilities */
        ...
        $('#span').on('click', function(e) {
            ...
            $(code.display.wrapper).height($(code.display.wrapper).height() + 150);
        });
